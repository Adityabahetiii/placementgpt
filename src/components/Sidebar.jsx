                ...
                sessions.map((session) => {
                  ...
                  return (
                    <div
                      key={session.id}
                      onClick={() => {
                        navigate(`/?chatId=${session.id}`);
                        if (setIsOpen) setIsOpen(false);
                      }}
                      className={`group relative flex w-full cursor-pointer items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm transition ${
                        isActive
