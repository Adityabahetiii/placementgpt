...
export function renderHero(roadmap, theme, width) {
  ...
  return `
    <g>
      ...
      <text x="90" y="135" fill="${theme.text}" font-size="54" font-weight="800">${title}</text>
      <rect x="90" y="162" width="240" height="48" rx="24" fill="${theme.accentSoft}" stroke="${theme.cardStroke}" />
      <text x="118" y="193" fill="${theme.accentTwo}" font-size="22" font-weight="700">${duration}</text>
      ${summaryLines.map((line, index) => `<text x="90" y="${272 + index * 30}" fill="${theme.muted}" font-size="22" font-weight="400">${escapeXml(line)}</text>`).join("")}
    </g>
  `;
}
